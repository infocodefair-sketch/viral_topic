      ...
      <div className="mb-4 flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <p className="mt-2 max-w-2xl text-sm leading-6 text-neutral-400">
            Fresh image drops with quick captions, curated for fast browsing between video sessions.
          </p>
        </div>
      </div>
      ...
      {images.length > 0 ? (
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {images.map((image, index) => (
            <article key={image.id} className="group overflow-hidden rounded-lg border border-white/10 bg-white/[0.04]">
